...
public class NodeChain {
	...
	public static class VarResult
	{
		...
		public HashMap <String, VarResult> mapResult;
		public boolean mapResultFound;
		...
		public VarResult()
		{
			...
			mapResult = null;
			mapResultFound = false;
		}
		
		public String toString()
		{
			HashMap <String, String> strs = toStrings("");
			if (strs.isEmpty()) 
			{
				return "[no value]";			
			}
			...
			{
				return strs.get(strs.keySet().iterator().next());
			}
		}
		
		public HashMap<String, String> toStrings(String name)
		{
			//ArrayList<String> outStrings = new ArrayList<String>();
			HashMap<String, String> outMap = new HashMap<String, String>();
			if (intResultFound) {
				outMap.put(name, "" + intResult);
			}
			else if (stringResultFound) {
				outMap.put(name, stringResult);
			}
			else if (ASTNodeResultFound) {
				outMap.put(name, ASTNodeResult.toString());
			}
			else if (mapResultFound) {
				for (String key : mapResult.keySet())
				{
					String outKey = name + "*" + key;
					String outVal = mapResult.get(key).toString();
					outMap.put(outKey,  outVal);
				}
			}
			else outMap.put(name, "[no value]");
		...
		{
			return (stringResultFound || intResultFound || ASTNodeResultFound || mapResultFound);
		}
	}
	...
	public static final int FIRSTCHILD = 5;	
...
//			return varResult;
//		}
		if ((l.stringResultFound && r.ASTNodeResultFound && r.ASTNodeResult.node != null && r.ASTNodeResult.node instanceof SimpleName) ||
				(r.stringResultFound && l.ASTNodeResultFound  && l.ASTNodeResult.node != null && l.ASTNodeResult.node instanceof SimpleName))
			{
				if (l.stringResultFound)
				{				
					vr = applyStringOperation(l.stringResult, ((SimpleName)r.ASTNodeResult.node).getIdentifier(), operation);
				}
				else
				{
					vr = applyStringOperation(((SimpleName)l.ASTNodeResult.node).getIdentifier(), r.stringResult, operation);
				}
			}
		else if ((l.intResultFound && r.ASTNodeResultFound) ||
	...
	static VarResult evaluateFunction(CommonTree ct, LinkedList <NodeChain> bindings, LinkedList <NodeChain> locals, Query currentQuery, ResultTree result)
	{
		...
		if (functionName.equals("depth") || functionName.equals("methodbinding") || functionName.equals("nodetype"))
		{
			...
			String argName = arg_node_chain_ct.getText().toString();
			...
			if (functionName.equals("nodetype"))
			{
				...
				{
					if (nc.localVarValue.ASTNodeResult == null ||
						nc.localVarValue.ASTNodeResult.node == null)						
					{
						Assert.isTrue(false, functionName + " only functions on an ASTNode (and not an AST Node List or anything else).");
					}
					
					varResult.stringResult = nc.localVarValue.ASTNodeResult.node.getClass().getName();					
				}
				else
				{
			...
			else
			{
				...
				else if (functionName.equals("methodbinding"))
				{
					...
					if (functionName.equals("methodbinding"))
					{
		...
		else if (functionName.equals("map"))
		{
			...
			CommonTree vr1 = (CommonTree)functionNode.getChild(0);
			
			VarResult vr2 = evaluateVar((CommonTree)functionNode.getChild(1), bindings, locals, true, currentQuery, result);
			VarResult vr3 = evaluateVar((CommonTree)functionNode.getChild(2), bindings, locals, true, currentQuery, result);

			NodeChain lvalue = 	QueryHandler.GetSearchNode(vr1, 0);
			...
			if (boundNC == null)
			{
				...
				boundNC.localVarValue = new VarResult();
				boundNC.localVarValue.mapResultFound = true;
				boundNC.localVarValue.mapResult = new HashMap <String, VarResult> ();
				newVar = true;
			}
			
			if (boundNC.localVarValue == null ||
				boundNC.localVarValue.mapResultFound == false)
			{
				Assert.isTrue(false, "Variable isn't a map!");
			}
			...
			if (!vr2.stringResultFound)
			{
				Assert.isTrue(false, "Key added to a a map must be a string");
			}
			
			boundNC.localVarValue.mapResult.put(vr2.stringResult, vr3);
			
		    varResult = vr3;
		}
		else if (functionName.equals("mapget"))
		{
			boolean newVar = false;
			CommonTree vr1 = (CommonTree)functionNode.getChild(0);
			...
			if (!vr2.stringResultFound)
			{
				Assert.isTrue(false, "Key to a map must be a string");
			}
			
			vrOut = (VarResult)boundNC.localVarValue.mapResult.get(vr2.stringResult);
			if (vrOut == null)
			{
				vrOut = new VarResult();
				vrOut.intResult = 0;
				vrOut.intResultFound = true;
			}
				...
				if (nc != null)
				{
					...
					else // the value of this variable is an AST tree or a simple structural property
					{
						...
						if (nodeChain.contains != null || nodeChain.containsType == LINENUMBER || 
							nodeChain.containsType == NEXTNODE || nodeChain.containsType == FIRSTCHILD)
						{
							...
							else if (nodeChain.containsType == NEXTNODE)
							{
								ASTNodeOrNodeList nextNode = currentResult.getNextNode();
								varResult.empty();
								if (nextNode != null)
								{								
									varResult.ASTNodeResultFound = true;
									varResult.ASTNodeResult = nextNode;
								}
								else
								{
									varResult.intResultFound = true;
							...
							else if (nodeChain.containsType == FIRSTCHILD)
							{
								ASTNodeOrNodeList firstChild = currentResult.getFirstChild();
								varResult.empty();
								if (firstChild != null)
								{								
									varResult.ASTNodeResultFound = true;
									varResult.ASTNodeResult = firstChild;
								}
								else
								{
									varResult.intResultFound = true;
