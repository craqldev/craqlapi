...
//	}
			...
			for ( int i = 0; i < t.getChildCount(); i++ ) {
				...
				else if (childStr.equals("AST_CHILD"))
				{
					...
					//System.out.println(sb.toString() + "ast_child: " + query.searchNodeType);					
						 ...
						 childStr.equals("nextnode") ||
						 childStr.equals("firstchild"))
				{
					//dummy3();
					...
					else if (childStr.equals("nextnode"))
					{
						containsType = NodeChain.NEXTNODE;
					}
					else if (childStr.equals("firstchild"))
					{
						containsType = NodeChain.FIRSTCHILD;
					}
					else
					{
					...
					if (containsType == NodeChain.LINENUMBER || containsType == NodeChain.FIRSTCHILD || containsType == NodeChain.NEXTNODE)
					{
						containsNC = null;
					}
					else
	...
	public static void writeLocals(String filename) {
		...
		try
		{
			...
			{
				HashMap <String, String> strs = nc.localVarValue.toStrings(nc.name);
				Iterator<String> strIter = strs.keySet().iterator();
